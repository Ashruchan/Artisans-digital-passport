...
import { getPublicPassport } from "../api/client";
...
export default function PassportPage() {
  ...
  if (error || !passport) {
    return (
      <div className="min-h-screen bg-[#FAF3E9] px-6 py-16 text-center">
        <p className="text-2xl font-bold text-[#A83E3E] mb-4">
          {error || "Passport not found"}
        </p>
        <Link
          to="/"
          className="inline-block bg-[#3E5641] text-[#FAF3E9] px-8 py-4 rounded-2xl text-lg font-semibold"
        >
          Go home
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAF3E9] text-[#2B2420]">
      <nav className="px-6 py-5 flex items-center justify-between">
        <Link to="/" className="text-2xl font-bold text-[#3E5641]">
          Karigar
        </Link>
        <Link
          to="/"
      ...
      <div className="max-w-xl mx-auto px-6 pb-16 space-y-6">
        <div className="rounded-3xl overflow-hidden border border-[#2B2420]/10 bg-[#FAF3E9] shadow-sm">
          ...
          <div className="p-6 space-y-3">
            ...
            <p className="text-sm text-[#2B2420]/60 break-all">
              Passport ID: {passport.passportId || passport.id}
            </p>
          </div>
        </div>
        ...
        <section className="rounded-3xl border border-[#2B2420]/10 p-6 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-[#2B2420]/70">Made by</p>
              <p className="text-xl font-bold">{passport.artisan?.name || "Artisan"}</p>
        ...
        {passport.craftStory || passport.description ? (
          <section className="rounded-3xl border border-[#2B2420]/10 p-6">
            <div className="flex items-center gap-2 mb-3 text-[#C1613C]">
              ...
              <h2 className="text-lg font-bold text-[#2B2420]">Craft story</h2>
            </div>
        ...
        <section className="rounded-3xl border border-[#2B2420]/10 p-6 bg-white text-center">
          <h2 className="text-lg font-bold mb-4 text-[#3E5641]">Scan again</h2>
